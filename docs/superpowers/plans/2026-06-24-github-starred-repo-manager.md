...
- [x] **Step 3: 手动验证 — 配置真实 token 后同步成功**
...
- [x] **Step 2: 实现 repos/[id]/tags/route.ts**
...
全部 19 个 Task 已在 `main` 分支上直接执行完成（用户已明确同意不开 worktree/分支）。

### 计划外发现并修复的两个问题

1. **antd 6 与 Next.js App Router RSC 的兼容性问题**：根 layout 因要导出 `metadata` 必须是 Server Component，但 `antd` 整体以 `"use client"` 模块分发；`Layout.Header`/`Layout.Content`/`Menu` 这类"复合组件"的静态子属性（如 `Layout.Content`）在 Server Component 里访问对应的 Client 引用代理时会变成 `undefined`，导致 `/`、`/repos`、`/stars` 全部 500（`Element type is invalid ... got: undefined`）。
   修复：新增 `app/components/AppShell.tsx`（Client Component）承载所有用到 antd 复合组件的部分，根 `layout.tsx` 保留为 Server Component 只做 `metadata` 导出和拼装。

2. **新版 `eslint-plugin-react-hooks`（v7，基于 React Compiler 的诊断）新增的 `set-state-in-effect` 规则**：只要 `useEffect` 回调（包括其调用的本地函数）在任意位置调用了 `setState`，无论是否在 `await` 之后，都会报错——这会拦截最常见的"在 effect 里 fetch 数据再 setState"模式。
   修复：在 `app/components/RepoList.tsx` 的两处 `fetchTags()`/`fetchRepos()` 调用上加 `eslint-disable-next-line react-hooks/set-state-in-effect` 并注明原因，未改变实现方式。

两者都源于 `AGENTS.md` 提到的"这不是你训练数据里熟悉的 Next.js"——具体表现为 antd 6 + Next.js 16 + 新版 eslint-plugin-react-hooks 的组合在训练数据截止时并不存在或行为不同。

### 验证结果

- `npx tsc --noEmit`：无报错
- `npm run lint`：无报错（除一条与本项目代码无关、且已被 `.gitignore` 排除的 `.remember/tmp` 警告）
- `npm run test`：6 个测试文件、34 个测试全部通过
- `npm run build`：生产构建成功
- 原始需求 1-9 条逐条核对：
  - 需求 2/3/4/5/6/7/8：用 `curl` 直接调用 `GET/PATCH/PUT /api/*` 验证，包括 owned/starred 数据集不同、分页不重复、收藏/备注/标签写入后能被对应筛选条件精确命中
  - 需求 9（star/unstar 真实生效）：用 `octocat/Spoon-Knife` 做了一次真实 star→确认 GitHub 端 204→unstar→确认 GitHub 端 404 的完整闭环，测试数据已还原
  - 需求 1（移动端响应式）以及所有依赖真实点击/拖拽/视口缩放的交互（筛选下拉、标签多选输入、备注弹窗、分页点击、F12 切手机视口）**未做自动化验证**——当前环境没有浏览器自动化工具，这部分需要用户在浏览器里手动走一遍 `http://localhost:6602`。
